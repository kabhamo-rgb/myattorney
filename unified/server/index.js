...
app.post('/api/client/otp/request', async (req, res) => {
  ...
  if (SMS_CONFIGURED && !sent) {
    ...
    otpStore.delete(key)
    // TEMP DIAGNOSTIC: surface the provider's exact error so we can pinpoint the cause. Remove after fix.
    res.status(502).json({ sent: false, error: 'שליחת הקוד נכשלה — ' + (lastSmsError || 'סיבה לא ידועה') })
    return
  }
