...
const pricingTiers = [
  ...
  },
  {
    ...
    price: '₪399',
    sub: __t("לטופס בודד · לא כולל מע״מ · אינו כולל אגרות או כל תשלום אחר"),
  ...
  },
  {
    ...
    price: '₪899',
    sub: __t("לתיק שלם · לא כולל מע״מ · אינו כולל אגרות או כל תשלום אחר"),
...
const legalTopics: { match: string[]; label: string; sources: { t: string; u: string }[] }[] = [
  {
    ...
    sources: [
      ...
      { t: __t("מדריך לחייב בהוצאה לפועל שיש עליו עיקולים (כל זכות)"), u: KZ + '%D7%94%D7%95%D7%A6%D7%90%D7%94_%D7%9C%D7%A4%D7%95%D7%A2%D7%9C_%D7%95%D7%92%D7%91%D7%99%D7%99%D7%94' },
    ],
  },
  {
...
const formLinksByLabel: Record<string, { t: string; u: string }[]> = {
  'עיקולים והוצאה לפועל': [
    ...
    { t: __t("בקשה בטענת \"פרעתי\" / השבת כספים — מדריך (כל זכות)"), u: KZ + '%D7%94%D7%95%D7%A6%D7%90%D7%94_%D7%9C%D7%A4%D7%95%D7%A2%D7%9C_%D7%95%D7%92%D7%91%D7%99%D7%99%D7%94' },
  ],
  'דיני עבודה ופיטורים': [
    { t: __t("תביעה בבית הדין לעבודה — מידע וטפסים (gov.il)"), u: 'https://www.court.gov.il/' },
  ],
}
...
type QaTopic = {
  ...
  keywords: string[]
  recs: string[]
  laws: { law: string; explanation: string }[]
  rights: string[]
  docs: string[]
  deadlines: string[]
  mistakes: string[]
...
const buildImmediateQuestionAssessment = (rawQuestion: string) => {
  ...
  const topics: QaTopic[] = [
    {
      ...
      recs: [
        __t("להוציא \"תדפיס תיק\" מלא ממערכת ההוצאה לפועל ולהשוות את יתרת החוב מול הסכומים שנגבו בפועל — לאיתור עיקול/גבייה ביתר."),
        __t("לוודא שלא עוקלו כספים מוגנים (שכר עד התקרה המוגנת, קצבאות ביטוח לאומי, דמי מזונות)."),
        __t("לבדוק אם צו העיקול הומצא כדין ואם חושבו ריבית והצמדה כדין."),
        __t("לשקול הגשת בקשה לרשם ההוצאה לפועל להשבת כספים שנגבו ביתר, לעיכוב הליכים או לפריסת החוב."),
      ],
      laws: [
        { law: __t("חוק ההוצאה לפועל, התשכ״ז-1967 — סעיף 19 (טענת \"פרעתי\")"), explanation: __t("מאפשר לחייב לטעון ולהוכיח שהחוב שולם, כולו או חלקו, ולעצור/לבטל גבייה שאינה מוצדקת.") },
        { law: __t("חוק הגנת השכר, התשי״ח-1958 — סעיף 8"), explanation: __t("קובע כי חלק מהשכר מוגן מעיקול — לא ניתן לעקל את המשכורת כולה, אלא רק מעל תקרה מסוימת.") },
        { law: __t("חוק הביטוח הלאומי [נוסח משולב], התשנ״ה-1995"), explanation: __t("קצבאות רבות (הבטחת הכנסה, נכות, זקנה ועוד) מוגנות מעיקול, כולן או חלקן.") },
        { law: __t("חוק ההוצאה לפועל — פרקי העיקול והגבלת החייב"), explanation: __t("מסדירים את הליכי העיקול, הגבלת חשבון/רישיון, ואת הדרך לבקש עיכוב, ביטול או הפחתה.") },
      ],
      rights: [
        __t("הזכות לעיין בתיק ההוצאה לפועל שלך ולקבל פירוט חיובים מלא."),
        __t("הזכות שלא יעוקלו כספים המוגנים בחוק (שכר עד תקרה, קצבאות, מזונות)."),
        __t("הזכות לטעון \"פרעתי\" ולהביא ראיות לתשלום."),
        __t("הזכות לבקש פריסת תשלומים, הפחתת צו חיוב או עיכוב הליכים לפי נסיבותיך."),
      ],
      docs: [__t("תדפיס תיק מלא מההוצאה לפועל"), __t("צו העיקול / האזהרה שקיבלת"), __t("אסמכתאות על כל תשלום ששילמת"), __t("תלושי שכר או אישור קצבה (להוכחת כספים מוגנים)")],
      deadlines: [__t("חוב פסוק מתיישן בדרך כלל לאחר 25 שנה, אך אין להמתין — עיקול פעיל גובה עכשיו."), __t("טענת \"פרעתי\" — אין מועד קבוע, אך מומלץ להגיש מיד עם גילוי גבייה ביתר כדי לעצור המשך גבייה.")],
      mistakes: [__t("להתעלם מאזהרה — הדבר מוביל לעיקולים והגבלות."), __t("לשלם בלי לקבל אסמכתא/קבלה."), __t("לא לבדוק אם עוקלו כספים מוגנים או אם הריבית חושבה כדין.")],
      expect: __t("בדרך כלל: הוצאת תדפיס תיק → בדיקת חישוב מול הסכומים שנגבו → הגשת בקשה מתאימה (טענת פרעתי / השבת כספים / עיכוב). משך הטיפול תלוי בסוג התיק ובתגובת הגורמים."),
    },
    {
      ...
      recs: [
        __t("לבדוק זכאות לפיצויי פיטורים, הודעה מוקדמת, פדיון חופשה, דמי הבראה והפרשות פנסיה."),
        __t("לבדוק אם נערך שימוע כדין לפני הפיטורים (זכות טיעון אמיתית)."),
        __t("לאסוף תלושי שכר, הסכם העסקה, מכתב הפיטורים והתכתבויות."),
        __t("לבחון תביעה לבית הדין לעבודה אם נפגעו זכויות."),
      ],
      laws: [
        { law: __t("חוק פיצויי פיטורים, התשכ״ג-1963"), explanation: __t("עובד שפוטר לאחר שנה ברציפות זכאי בדרך כלל לפיצויי פיטורים בשיעור שכר חודש לכל שנת עבודה.") },
        { law: __t("חוק הודעה מוקדמת לפיטורים ולהתפטרות, התשס״א-2001"), explanation: __t("מחייב מתן הודעה מוקדמת (או תשלום חלף הודעה) לפי ותק העובד.") },
        { law: __t("חוק הגנת השכר, התשי״ח-1958"), explanation: __t("קובע מועדי תשלום שכר ופיצויי הלנת שכר על איחור בתשלום.") },
        { law: __t("הלכת השימוע (פסיקת בתי הדין לעבודה)"), explanation: __t("מעסיק חייב לערוך שימוע הוגן לפני פיטורים; היעדר שימוע כדין עשוי להקים עילה לפיצוי.") },
      ],
      rights: [
        __t("הזכות לפיצויי פיטורים והודעה מוקדמת (בכפוף לתנאי הזכאות)."),
        __t("הזכות לשימוע הוגן לפני פיטורים."),
        __t("הזכות לפדיון ימי חופשה, דמי הבראה והפרשות פנסיה."),
        __t("הגנה מיוחדת בפיטורים בהיריון, חופשת לידה או לאחר תאונת עבודה."),
      ],
      docs: [__t("תלושי שכר (12 החודשים האחרונים)"), __t("הסכם/חוזה העסקה"), __t("מכתב הפיטורים והזימון לשימוע"), __t("התכתבויות עם המעסיק (וואטסאפ/מייל)")],
      deadlines: [__t("תביעה לזכויות עבודה מתיישנת בדרך כלל לאחר 7 שנים."), __t("תביעת הלנת שכר — התיישנות מקוצרת, חשוב לפעול מהר."), __t("ערעור/פנייה בעניין פיטורים בהיריון — יש לפעול בסמוך לאירוע.")],
      mistakes: [__t("לחתום על כתב ויתור / \"גמר חשבון\" לפני בדיקת הזכויות."), __t("לא לתעד שיחות והבטחות בכתב."), __t("לפספס מועד התיישנות, במיוחד בהלנת שכר.")],
      expect: __t("בדרך כלל: איסוף תלושים ומסמכים → חישוב הזכויות המגיעות → מכתב דרישה למעסיק → ובמידת הצורך תביעה לבית הדין לעבודה."),
    },
    {
      ...
      recs: [
        __t("לבדוק את סעיפי ההפרה, הפיצוי המוסכם ותנאי היציאה/הביטול בהסכם."),
        __t("לתעד את ההפרה ולשלוח דרישה/התראה מסודרת בכתב לפני נקיטת צעד."),
        __t("לבדוק מועדי התיישנות ותנאים מוקדמים לביטול."),
      ],
      laws: [
        { law: __t("חוק החוזים (חלק כללי), התשל״ג-1973"), explanation: __t("מסדיר כריתת חוזה, פגמים ברצון (הטעיה, עושק, כפייה) וביטול חוזה בשל פגמים אלה.") },
        { law: __t("חוק החוזים (תרופות בשל הפרת חוזה), התשל״א-1970"), explanation: __t("קובע את הסעדים בהפרת חוזה: אכיפה, ביטול, והשבה, וכן פיצויים על הנזק שנגרם.") },
        { law: __t("חוק השכירות והשאילה, התשל״א-1971"), explanation: __t("חל על הסכמי שכירות ומסדיר את חובות וזכויות המשכיר והשוכר.") },
      ],
      rights: [
        __t("הזכות לתבוע אכיפת החוזה, ביטולו, או פיצוי בגין ההפרה."),
        __t("הזכות לפיצוי מוסכם אם נקבע בחוזה (בכפוף לסבירות)."),
        __t("הזכות לבטל חוזה שנכרת בהטעיה, עושק או כפייה."),
      ],
      docs: [__t("החוזה המלא + נספחים"), __t("כל ההתכתבויות סביב ההסכם"), __t("אסמכתאות תשלום/העברות"), __t("תיעוד ההפרה (תמונות, הודעות, עדויות)")],
      deadlines: [__t("תביעה חוזית מתיישנת בדרך כלל לאחר 7 שנים."), __t("לרוב יש לשלוח התראה סבירה בכתב לפני ביטול או תביעה.")],
      mistakes: [__t("לבטל חוזה בלי התראה מוקדמת כנדרש."), __t("לא לתעד את ההפרה בזמן אמת."), __t("להסתמך על הבטחות בעל-פה שאינן בכתב.")],
      expect: __t("בדרך כלל: ניתוח סעיפי החוזה → תיעוד ההפרה → מכתב התראה/דרישה → משא ומתן, ובמידת הצורך תביעה."),
    },
    {
      label: __t("נזיקין ותאונות"),
      keywords: [__t("תאונה"), __t("נזק"), __t("פגיעה"), __t("ביטוח"), __t("פיצוי"), __t("רשלנות")],
      recs: [
        __t("לאסוף תיעוד רפואי, אישורי מחלה והוכחות נזק."),
        __t("לבדוק את אחריות הצד הפוגע ואת הכיסוי הביטוחי הרלוונטי."),
        __t("לבדוק את מועד ההתיישנות ולפעול בהקדם."),
      ],
      laws: [
        { law: __t("פקודת הנזיקין [נוסח חדש]"), explanation: __t("הבסיס לתביעות רשלנות ונזק — מי שהתרשל וגרם נזק עשוי לחוב בפיצוי.") },
        { law: __t("חוק הפיצויים לנפגעי תאונות דרכים, התשל״ה-1975 (פלת\"ד)"), explanation: __t("מקנה פיצוי לנפגעי תאונות דרכים ללא תלות באשם, דרך חברת הביטוח.") },
        { law: __t("חוק ההתיישנות, התשי״ח-1958"), explanation: __t("קובע את פרק הזמן שבו ניתן להגיש תביעה — בנזיקין בדרך כלל 7 שנים.") },
      ],
      rights: [
        __t("הזכות לפיצוי על נזק גוף ורכוש שנגרם ברשלנות הזולת."),
        __t("הזכות לפיצוי בגין הפסד השתכרות והוצאות רפואיות."),
        __t("בתאונת דרכים — פיצוי מחברת הביטוח גם ללא הוכחת אשם."),
      ],
      docs: [__t("כל התיעוד הרפואי ואישורי המחלה"), __t("דוח משטרה / טופס הודעה על תאונה"), __t("קבלות על הוצאות"), __t("צילומים ותיעוד של הנזק והנסיבות")],
      deadlines: [__t("תביעת נזיקין מתיישנת בדרך כלל לאחר 7 שנים (בקטין — עד גיל 25)."), __t("תאונת עבודה — יש לדווח לביטוח הלאומי בהקדם."), __t("תאונת דרכים — לתעד ולפנות לטיפול רפואי מיד.")],
      mistakes: [__t("לא לתעד את הנזק והפציעה מיד לאחר האירוע."), __t("לחתום על פשרה מול חברת ביטוח בלי בדיקה."), __t("לפספס את מועד ההתיישנות.")],
      expect: __t("בדרך כלל: איסוף תיעוד רפואי → הערכת הנזק והאחריות → פנייה לחברת הביטוח/הצד האחראי → ובמידת הצורך תביעה."),
    },
  ]
  ...
  return {
    ...
    summary: matched
      ...
      "\". להלן בדיקה ראשונית מפורטת: החוקים הרלוונטיים, הזכויות שלך, מסמכים לאיסוף, מועדים חשובים וטעויות שכדאי להימנע מהן. אינה תחליף לייעוץ משפטי מלא."
    ], matched.label)
      : __t(
      "זוהתה שאלה משפטית כללית. להלן כיווני בדיקה ראשוניים; לתשובה מדויקת נדרשת בחינה של המסמכים והנסיבות. ניתן להעלות מסמך לבדיקה מעמיקה יותר."
    ),
    findings: [
      ...
      __t("הבדיקה מבוצעת באופן מיידי לפי ניתוח מילולי של השאלה, כשלב מקדים לבדיקה מעמיקה."),
    ],
    recommendations: matched ? matched.recs : generalRecs,
    relevantLaws: matched ? matched.laws : undefined,
    rights: matched ? matched.rights : undefined,
    documentsNeeded: matched ? matched.docs : [__t("הסכם/מכתב/דרישה רלוונטיים"), __t("אסמכתאות תשלום"), __t("כל התכתבות עם הצד השני")],
    deadlines: matched ? matched.deadlines : [__t("בדוק מועד התיישנות — ברוב התחומים 7 שנים; בחלקם קצר יותר.")],
    commonMistakes: matched ? matched.mistakes : [__t("לחתום על מסמך/ויתור לפני בדיקה."), __t("לא לתעד בכתב."), __t("להמתין ולפספס מועדים.")],
    whatToExpect: matched ? matched.expect : __t("בדרך כלל: איסוף מסמכים → הערכת הזכויות והסיכונים → מכתב/פנייה מסודרת → ובמידת הצורך הליך משפטי."),
    riskLevel,
    nextStep: __t(
      "זוהי הערכה ראשונית אוטומטית ולא ייעוץ משפטי מחייב. להמשך — ניתן להעלות מסמך לבדיקה מעמיקה, לפנות לשירות מלא, או להזמין הכנת טפסים ושליחתם."
    ),
  };
}
...
const buildGarnishmentAssessment = (input: GarnishmentInput) => {
  ...
  const relevantLaws = [
    { law: __t("חוק ההוצאה לפועל, התשכ״ז-1967 — סעיף 19 (טענת \"פרעתי\")"), explanation: __t("מאפשר לחייב לטעון ולהוכיח ששילם את החוב, ולעצור גבייה שאינה מוצדקת.") },
    { law: __t("חוק הגנת השכר, התשי״ח-1958 — סעיף 8"), explanation: __t("חלק מהמשכורת מוגן מעיקול — לא ניתן לעקל את השכר כולו אלא רק מעל תקרה.") },
    { law: __t("חוק הביטוח הלאומי [נוסח משולב], התשנ״ה-1995"), explanation: __t("קצבאות רבות מוגנות מעיקול — אם עוקלו, ייתכן שניתן להשיבן.") },
  ]
    ...
    <div className={route === 'client' ? 'page-shell client-mode' : 'page-shell'}>
      ...
      <header className="topbar">
        ...
        <nav className="main-nav" aria-label={__t("כותרת עיקרית")}>
          ...
          <a href="#pricing">{__t("עלות השירותים")}</a>
      ...
      <main>
          ...
          <div className="tool-card">
            ...
            {heroTab === 'garnish' && garnishResult && (
              <div className="review-result garnish-result" aria-live="polite">
                ...
                {Array.isArray(garnishResult.relevantLaws) && garnishResult.relevantLaws.length > 0 && (
                  <div className="result-block laws-block"><strong>{__t("⚖️ החוקים והסעיפים הרלוונטיים")}</strong>
                    <ul className="laws-list">{garnishResult.relevantLaws.map((l, i) => (
                      <li key={i}><span className="law-name">{l.law}</span>{l.explanation ? <span className="law-exp"> — {l.explanation}</span> : null}</li>
                    ))}</ul>
                  </div>
                )}
                <div className="result-block">
                  <strong>{__t("שלבי טיפול מומלצים")}</strong>
            ...
            {heroTab !== 'garnish' && reviewResult && (
              <div className="review-result" aria-live="polite">
                ...
                {Array.isArray(reviewResult.relevantLaws) && reviewResult.relevantLaws.length > 0 && (
                  <div className="result-block laws-block"><strong>{__t("⚖️ החוקים והסעיפים הרלוונטיים")}</strong>
                    <ul className="laws-list">{reviewResult.relevantLaws.map((l, i) => (
                      <li key={i}><span className="law-name">{l.law}</span>{l.explanation ? <span className="law-exp"> — {l.explanation}</span> : null}</li>
                    ))}</ul>
                  </div>
                )}

                {Array.isArray(reviewResult.rights) && reviewResult.rights.length > 0 && (
                  <div className="result-block"><strong>{__t("🛡️ הזכויות שלך")}</strong>
                    <ul>{reviewResult.rights.map((item) => (<li key={item}>{item}</li>))}</ul>
                  </div>
                )}
                ...
                {Array.isArray(reviewResult.documentsNeeded) && reviewResult.documentsNeeded.length > 0 && (
                  <div className="result-block"><strong>{__t("📎 מסמכים שכדאי לאסוף")}</strong>
                    <ul>{reviewResult.documentsNeeded.map((item) => (<li key={item}>{item}</li>))}</ul>
                  </div>
                )}

                {Array.isArray(reviewResult.deadlines) && reviewResult.deadlines.length > 0 && (
                  <div className="result-block deadlines-block"><strong>{__t("⏰ מועדים חשובים לתשומת לב")}</strong>
                    <ul>{reviewResult.deadlines.map((item) => (<li key={item}>{item}</li>))}</ul>
                  </div>
                )}

                {reviewResult.whatToExpect && (
                  <div className="result-block"><strong>{__t("🔎 מה צפוי בהמשך")}</strong><p>{reviewResult.whatToExpect}</p></div>
                )}

                {Array.isArray(reviewResult.commonMistakes) && reviewResult.commonMistakes.length > 0 && (
                  <div className="result-block mistakes-block"><strong>{__t("⚠️ טעויות נפוצות שכדאי להימנע מהן")}</strong>
                    <ul>{reviewResult.commonMistakes.map((item) => (<li key={item}>{item}</li>))}</ul>
                  </div>
                )}
        ...
        <section id="pricing" className="section pricing-section" aria-label={__t("עלות השירותים")}>
          <div className="section-header">
        ...
        <section id="privacy" className="section legal-section">
          ...
          <div className="legal-body">
            <p className="legal-updated">{__t("עודכן: אוגוסט 2026")}</p>
            <p><strong>{__t("בעל השליטה במידע:")}</strong>{" " + __t(
              "משרד עורכי דין מוחמד מ. קבהא (מ.ר 67912) («המשרד»). כתובת: בסמ״ה, רח' אלבוח'ארי 95, מיקוד 3002300. נציב פניות פרטיות: עו״ד מוחמד קבהא · info@my-attorney.net · 052-661-1866."
            )}</p>
            ...
            <p><strong>{__t("סוגי מידע:")}</strong>{" " + __t(
              "פרטי זיהוי והתקשרות (שם, ת״ז, טלפון, דוא״ל), מסמכים ונתונים שתמסרו לצורך הבדיקה, נתוני התיק, ומידע טכני בסיסי (כתובת IP, סוג דפדפן). מסירת המידע תלויה בהסכמתכם; אינכם חייבים למסור מידע, אך ללא מידע מסוים לא נוכל לספק את השירות."
            )}</p>
            <p><strong>{__t("ספקי עיבוד (Processors):")}</strong>{" " + __t(
              "לצורך הפעלת השירות אנו נעזרים בספקים: אחסון והרצה בענן (Railway), ניתוח וסיווג באמצעות בינה מלאכותית (Google Gemini), משלוח דוא״ל (Resend), סליקת תשלומים (Stripe), והפקת חשבוניות (Morning / חשבונית ירוקה). חלק מהספקים מפעילים שרתים מחוץ לישראל (בין היתר בארה״ב ובאיחוד האירופי)."
            )}</p>
            <p><strong>{__t("בינה מלאכותית:")}</strong>{" " + __t(
              "עיבוד באמצעות AI נעשה לצורך המיון והבדיקה בלבד. המשרד אינו מתיר שימוש בתוכן שהעליתם לאימון מודלים של בינה מלאכותית, בהתאם לתנאי הספקים. אין להעלות מסמכים שאינם נחוצים לבדיקה."
            )}</p>
            <p><strong>{__t("העברת מידע לחו״ל:")}</strong>{" " + __t(
              "ההעברה נעשית לצורך אספקת השירות ובהסכמתכם, בהתאם לתקנות הגנת הפרטיות (העברת מידע אל מאגרי מידע שמחוץ לגבולות המדינה), התשס״א-2001, ובכפוף להתחייבות הספקים לרמת הגנה נאותה. איננו מוכרים מידע ואיננו מעבירים אותו לצד שלישי שלא לצורך השירות, למעט כנדרש על פי דין."
            )}</p>
            <p><strong>{__t("משכי שמירה:")}</strong>{" " + __t(
              "ליד (פנייה ראשונית) שלא הבשיל להתקשרות — יישמר עד 12 חודשים ואז יימחק; מסמך שהועלה לבדיקה ולא הפך לתיק — עד 90 יום; תיק לקוח פעיל או סגור — יישמר 7 שנים לפחות ממועד סיום הטיפול, בהתאם לחובות הדין וכללי לשכת עורכי הדין. (המשכים ניתנים לעדכון לפי מדיניות המשרד.)"
            )}</p>
            <p><strong>{__t("ליד מול תיק פעיל:")}</strong>{" " + __t(
              "פנייה ראשונית (ליד) מנוהלת בנפרד מתיק משפטי פעיל. יחסי עו״ד–לקוח וחיסיון מקצועי נוצרים רק עם חתימת הסכם התקשרות; מידע בליד שלא הבשיל אינו מנוהל כתיק לקוח."
            )}</p>
            <p><strong>{__t("מידע של צד שלישי:")}</strong>{" " + __t(
              "אין להעלות מסמכים או פרטים של אדם אחר שאינם נחוצים לבדיקה. ככל שמידע כזה נמסר, הוא יעובד למטרת הבדיקה בלבד, ייחשף רק לגורמים הנדרשים, וימחק אם אינו נדרש עוד."
            )}</p>
            <p><strong>{__t("עוגיות ומדידה:")}</strong>{" " + __t(
              "האתר עושה שימוש מזערי באחסון מקומי בדפדפן (localStorage) לצורך תפעול בלבד — העדפת שפה, סימון הסכמה ומזהה תיק. איננו עושים שימוש בעוגיות פרסום או במעקב שיווקי של צד שלישי."
            )}</p>
            <p><strong>{__t("זכות עיון, תיקון ומחיקה:")}</strong>{" " + __t(
              "לפי סעיפים 13–14 לחוק, אתם רשאים לעיין במידע שלכם ולבקש את תיקונו או מחיקתו. לבקשה ייעודית פנו לנציב הפניות: info@my-attorney.net — נטפל בפנייה בתוך פרק זמן סביר וכנדרש בדין."
            )}</p>
            <p><strong>{__t("אבטחת מידע ונוהל אירוע אבטחה:")}</strong>{" " + __t(
              "אנו נוקטים אמצעים לאבטחת המידע (הצפנת תעבורה HTTPS, הרשאות והגבלת גישה, אחסון סיסמאות כ-hash). בהתרחש אירוע אבטחה מהותי — ננקוט צעדי בלימה, נתעד את האירוע ונודיע לנפגעים ולרשות להגנת הפרטיות ככל שנדרש על פי דין."
            )}</p>
            <p><strong>{__t("הסכמה:")}</strong>{" " + __t(
              "עיבוד מסמכים ופרטים ייעשה רק לאחר סימון מפורש של תיבת ההסכמה הרלוונטית. עצם השימוש בכלים אינו מהווה כשלעצמו הסכמה לעיבוד מסמך. מידע נוסף: הרשות להגנת הפרטיות."
            )}</p>
          </div>
        </section>
        ...
        <section id="terms" className="section legal-section">
          ...
          <div className="legal-body">
            <p className="legal-updated">{__t("עודכן: אוגוסט 2026")}</p>
            <p><strong>{__t("פרטי העוסק:")}</strong>{" " + __t(
              "משרד עורכי דין מוחמד מ. קבהא · עוסק מורשה · עו״ד מוחמד קבהא, מ.ר 67912 · בסמ״ה, רח' אלבוח'ארי 95, מיקוד 3002300 · info@my-attorney.net · 052-661-1866."
            )}</p>
            ...
            <p><strong>{__t("מה מקבל הלקוח:")}</strong>{" " + __t(
              "מיון ראשוני אוטומטי ובדיקת זכאות ראשונית — ללא עלות; הכנת טופס משפטי או סט טפסים — בתשלום; טיפול בבקשת החזר עיקול — בהסכם שכר טרחה נפרד. השירות המדויק, היקפו ומחירו יפורטו טרם ההזמנה."
            )}</p>
            <p><strong>{__t("עלות השירותים:")}</strong>{" " + __t(
              "בדיקת/החזר עיקול — ללא עלות; שכר טרחה מותנה הצלחה 25% בתוספת מע״מ, מכל סכום שיושב בפועל בלבד («ללא זכייה — אין תשלום»). הכנת טפסים: ₪399 לטופס בודד, ₪899 לסט טפסים לתיק — המחירים אינם כוללים מע״מ, ואינם כוללים אגרות או כל תשלום אחר. המחיר הסופי נקבע לפי סוג התיק או הטופס. אגרות והוצאות חיצוניות יחולו על הלקוח בנפרד."
            )}</p>
            <p><strong>{__t("מועד תחילת השירות ואספקה:")}</strong>{" " + __t(
              "השירות מתחיל עם אישור ההזמנה או חתימת ההסכם. טופס מוכן נמסר בדרך כלל תוך 3 ימי עסקים; טיפול בהחזר עיקול מתחיל לאחר בדיקה אנושית ואישור."
            )}</p>
            <p><strong>{__t("סטטוס הטופס — «מוכן», «שליחה אונליין», «למשרד»:")}</strong>{" " + __t(
              "«טופס מוכן» = קובץ להורדה בלבד; «שליחה אונליין» = העברת הטופס ליעד הרשמי הרלוונטי; «שליחה למשרד» = הבקשה מגיעה למשרד בלבד ואינה מוגשת לרשות באופן אוטומטי. בכל מקרה יצוין לאן נשלח המסמך. אין ערובה לקבלת הבקשה על ידי הרשות."
            )}</p>
            <p><strong>{__t("ביטול עסקה (מכר מרחוק):")}</strong>{" " + __t(
              "בהתאם לחוק הגנת הצרכן, התשמ״א-1981 ותקנותיו, ניתן לבטל עסקת שירות בתוך 14 ימים ממועד ההתקשרות, ובלבד שהביטול ייעשה 14 ימים לפני מועד תחילת השירות. ביטול אפשרי בדוא״ל (info@my-attorney.net), בטלפון (052-661-1866), בטופס מקוון או בדואר לכתובת המשרד."
            )}</p>
            <p><strong>{__t("אוכלוסיות זכאיות לביטול מורחב:")}</strong>{" " + __t(
              "אדם עם מוגבלות, אזרח ותיק (בן 65 ומעלה) ועולה חדש רשאים לבטל עסקה בתוך 4 חודשים ממועד ההתקשרות, ובלבד שההתקשרות כללה שיחה (לרבות טלפונית) בין הצדדים."
            )}</p>
            <p><strong>{__t("דמי ביטול, ביצוע מיידי והחזר:")}</strong>{" " + __t(
              "ביטול לפני תחילת הביצוע — ללא דמי ביטול, למעט דמי ביטול כדין (עד 5% מהעסקה או 100 ₪, הנמוך מביניהם). ביקש הלקוח להתחיל בביצוע מיד — ייתכן חיוב חלקי בגין החלק שבוצע. הוכן טופס אך טרם נשלח — יינתן החזר בניכוי עלות ההכנה בפועל. נבצר מהמשרד לספק את השירות או אירעה תקלה טכנית — יינתן החזר מלא של הסכום ששולם עבורו."
            )}</p>
            ...
            <p><strong>{__t("דין וסמכות שיפוט:")}</strong>{" " + __t(
              "על השימוש יחולו דיני מדינת ישראל, וסמכות השיפוט הבלעדית נתונה לבתי המשפט המוסמכים בישראל."
            )}</p>
          </div>
        </section>
        ...
        <section id="accessibility" className="section legal-section">
          ...
          <div className="legal-body">
            <p><strong>{__t("מחויבות:")}</strong>{" " + __t(
              "המשרד רואה חשיבות רבה בהנגשת שירותיו לכלל הציבור, לרבות אנשים עם מוגבלות, ופועל למען עמידה בחוק שוויון זכויות לאנשים עם מוגבלות, התשנ״ח-1998, ובתקנות שוויון זכויות לאנשים עם מוגבלות (התאמות נגישות לשירות), התשע״ג-2013."
            )}</p>
            <p><strong>{__t("רמת ההנגשה ומועד הבדיקה:")}</strong>{" " + __t(
              "האתר נבנה בהתאם להמלצות התקן הישראלי ת״י 5568 (המבוסס על WCAG 2.0) ברמה AA. נבדק לאחרונה באוגוסט 2026 בבדיקה פנימית (מבנה סמנטי, ניגודיות צבעים, ניווט מלא במקלדת, מיקוד נראה, תוויות בטפסים ותמיכה בקוראי מסך)."
            )}</p>
            <p><strong>{__t("מגבלות ידועות:")}</strong>{" " + __t(
              "ייתכנו רכיבים, קבצים או מסמכים שאינם נגישים במלואם. אנו פועלים לשיפור מתמשך, ובכוונת המשרד לבצע בדיקת נגישות מקצועית חיצונית ולעדכן הצהרה זו בהתאם."
            )}</p>
            <p><strong>{__t("חלופה נגישה:")}</strong>{" " + __t(
              "נתקלתם בקושי בשימוש בכלי, בהעלאת מסמך, בחתימה או בתהליך התשלום? פנו לרכז הנגישות ונספק חלופה נגישה (טלפון, דוא״ל או פגישה) להשלמת הפעולה."
            )}</p>
            <p><strong>{__t("רכז הנגישות וזמן תגובה:")}</strong>{" " + __t(
              "רכז הנגישות: עו״ד מוחמד קבהא — info@my-attorney.net · 052-661-1866 · בסמ״ה, רח' אלבוח'ארי 95, מיקוד 3002300. נשתדל להשיב לפניות נגישות בתוך 14 ימי עסקים."
            )}</p>
          </div>
        </section>
      </main>
